...
param(
    ...
    [ValidateSet('doctor', 'install', 'status', 'version', 'help')]
    [string]$Command = 'help',
...
Usage:
  ...
  .\bin\sonarweaver.ps1 install docker [evaluation|production]
  .\bin\sonarweaver.ps1 status windows|docker
...
switch ($Command) {
    ...
    'status' {
        if ($Target -eq 'windows') {
            ...
            Push-Location (Join-Path $Root 'deployments\docker')
            try { & docker compose -f compose.yaml -f compose.local.yaml ps } finally { Pop-Location }
        } else {
